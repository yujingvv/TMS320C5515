        ...
        else
        {
            ...
            if (TXPKTRDY == 0x0000 && ep0_frame > 0) 
            {
                ...
                {   
                    ...
                    Desc_point = Desc_point + 64*ep0_frame;
                    fifo_write(Desc_point, 64);
                    ep0_frame++;
                    left_over = left_over - 64;
                    USB_PERI_CSR0 |= 0x20;  // Set TXPKTRDY (B1 of PERI_CSR0)
                } 
                else 
                {                     
                    // Current data sending will finish here
                    Desc_point = Desc_point + 64*ep0_frame;
                    fifo_write(Desc_point, left_over);
                    ep0_frame = 0;
                    left_over = 0;
...
void set_feature(void)
{
    ...
    if((bmRequestType==0x00) && (wValueL==0x02) && (wIndexH==0x01) && (wIndexL==0x00))
    {
        ...
        USB_PERI_CSR0 |= 0x80; //set DATAEND(B3 of PERI_CSR0)
